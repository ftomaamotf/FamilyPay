...
const rejectDisabledVoiceCall = (res) => (
  res.status(410).json({ success: false, message: 'ميزة الاتصال معطلة' })
);

// 9.1 Start Intercom Call
app.post('/api/intercom/call', (req, res) => {
  if (!VOICE_CALLS_ENABLED) return rejectDisabledVoiceCall(res);
});

// 9.2 Respond to Intercom Call
app.post('/api/intercom/respond', (req, res) => {
  if (!VOICE_CALLS_ENABLED) return rejectDisabledVoiceCall(res);
});

// 9.3 Send Voice Stream / Burst over Intercom
