...
export const PendingRequestsModal = ({ isOpen, onClose }) => {
  ...
  const handleSelectBankApp = (appId) => {
    ...
    setCopiedBankToast(`✅ تم نسخ رقم البطاقة وتحديد (${app?.shortName || appId})!`);
    setTimeout(() => setCopiedBankToast(''), 3500);
  };
  ...
  const handleApprove = async (e) => {
    ...
    if (res.success) {
      ...
      // Automatically launch the selected banking application on phone!
      launchBankApp({
      ...
      {selectedReq && actionType === 'approve' && (
        <div className="fixed inset-0 z-60 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fadeIn" dir="rtl">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl p-5 sm:p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-4 text-center text-slate-800 dark:text-slate-100">
            ...
            <div className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-right space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1">
                  <Smartphone className="w-3.5 h-3.5 text-emerald-600" />
                  <span>تطبيق التحويل البنكي المطلوب:</span>
                </span>
                <button
                  type="button"
                ...
                >
                  <span>{showAppPicker ? 'إغلاق' : 'تغيير التطبيق 📱'}</span>
              ...
              <div className="flex gap-2 pt-2">
                ...
                >
                  ...
                  <span>
                    {loading
                      ? 'جاري التوثيق والصرف...'
                      : `✅ تأكيد وصرف والانتقال إلى (${BANKING_APPS.find(a => a.id === selectedBankApp)?.shortName || 'تطبيق البنك'})`}
                  </span>
                </button>
                <button
                  type="button"
