...
import { Bell, X, Sparkles, Send, UserCheck, MessageSquare, MessageCircle } from 'lucide-react';

export const NotificationToast = ({ onOpenPendingRequests, onOpenGuestApprovals, onOpenChat }) => {
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5 font-black text-sm text-emerald-300">
            {isChatMessage ? <MessageCircle className="w-3.5 h-3.5" /> : <Sparkles className="w-3.5 h-3.5" />}
          ...
          {/* Quick Action Button for Chat Message */}
          {isChatMessage && onOpenChat && (
            <button
              ...
              className="mt-2.5 px-4 py-2 bg-gradient-to-r from-emerald-500 to-teal-400 hover:from-emerald-400 text-slate-950 font-black text-xs rounded-xl shadow-lg transition active:scale-95 flex items-center gap-1.5 cursor-pointer"
            >
              <MessageSquare className="w-3.5 h-3.5" />
              <span>الانتقال إلى المحادثة والرد الآن 💬</span>
            </button>
          )}
