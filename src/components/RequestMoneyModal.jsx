...
export const RequestMoneyModal = ({ isOpen, onClose, onSuccess = null, initialBrotherId = null, initialFieldId = null }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.success) {
      setSuccessMsg(res.message || '✅ تم إرسال طلبك بنجاح للأدمن مباشرة في النظام');
      setAmount('');
      setReason('');
      setCommodityName('');
      setTimeout(() => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fadeIn" dir="rtl">
      <div className="bg-white dark:bg-slate-800 w-full max-w-md rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 max-h-[90vh] flex flex-col overflow-hidden text-slate-800 dark:text-slate-100">
        ...
        <form onSubmit={handleSubmit} className="p-5 overflow-y-auto space-y-4 text-xs">
          ...
          {successMsg && (
            <div className="p-3 rounded-2xl bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 text-xs font-bold text-center flex items-center justify-center gap-1.5 animate-fadeIn">
              <CheckCircle2 className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
              <span>{successMsg}</span>
            </div>
          )}

          {/* Submit Action (مباشر من البرنامج للأدمن) */}
          <div className="pt-2">
            <button
              type="submit"
              disabled={!canSubmit}
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  <span>{isForGeneralExpenses ? 'إرسال طلب مصاريف عامة للأدمن 📦' : 'إرسال طلب الأموال للأدمن مباشرة 📤'}</span>
                </>
              )}
            </button>
            {!isReasonValid && (
