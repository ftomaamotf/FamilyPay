...
// ================= 9. Disabled In-App Voice Calling =================
const disabledIntercomResponse = (req, res) => {
  res.status(410).json({
    success: false,
    message: 'ميزة الاتصال الصوتي داخل التطبيق غير مفعلة حالياً. الدردشة والرسائل الصوتية ما زالت متاحة.'
  });
};

app.post('/api/intercom/call', disabledIntercomResponse);
app.post('/api/intercom/respond', disabledIntercomResponse);
app.post('/api/intercom/voice-burst', disabledIntercomResponse);
app.get('/api/intercom/status/:callId', disabledIntercomResponse);
app.get('/api/intercom/active-for/:userId', disabledIntercomResponse);
